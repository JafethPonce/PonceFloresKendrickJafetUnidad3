...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['g-recaptcha-response'])) {
        ...
        if ($responseData && $responseData['success']) {
            ...
            $psw = trim($_POST['psw']);
            $pregunta_secreta = trim($_POST['pregunta_secreta']);
            ...
            if (empty($errors)) {
                ...
                $sql = $cnnPDO->prepare("INSERT INTO Info_User (nombre, apellidos, celular, email, psw, id_rol, pregunta_secreta, respuesta_secreta) VALUES (:nombre, :apellidos, :celular, :email, :psw, :id_rol, :pregunta_secreta, :respuesta_secreta)");
                $sql->bindParam(':id_rol', $id_rol);
                $sql->bindParam(':nombre', $nombre);
                $sql->bindParam(':apellidos', $apellidos);
                $sql->bindParam(':celular', $celular);
                $sql->bindParam(':email', $email);
                $sql->bindParam(':psw', $hashedPassword); // Guardar la contraseña encriptada
                $sql->bindParam(':pregunta_secreta', $pregunta_secreta);
                $sql->bindParam(':respuesta_secreta', $respuesta_secreta);
                $sql->execute();
